...
float integration(vector< vector<float> > vecXY, int a, int b) {
	float h;
	if (a==0 && b==0) // Il faut faire une condition pour que ça marche dans ce cas précis.
		h=0;
	else
		h=vecXY[0][1]-vecXY[0][0]; // On définit le pas comme étant la distance entre 2 points de mon tableau.
...
/* On remplit un tableau avec des accélérations en fonction du temps, puis on intégre ces "points" deux fois.
 * Puis, on compare l'intégration double à l'intégrale exacte. */
